...
class Immune_systems():
    ...
    def immune(self, t, Immune):
        ...
        V_3_NabP = Immune[32]
        aPD1 = Immune[33]
        PDL1_aPDL1 = Immune[34]
        PDL1_aPDL1_PDL1 = Immune[35]
        ...
        if self.nabp != 0:
            k_C1_therapy = RC["k_C_nabp"] * (V_T_NabP / (V_T_NabP + RC["IC50_nabp"])) * min(V_T_C,
                                                                                            RC["Kc_nabp"]) / V_T_C
        else:
            k_C1_therapy = 0
        ...
        RC["H_TGFb_Teff"] = RC["V_T_TGFb"] / \
            (RC["V_T_TGFb"] + RC["TGFb_50_Teff"])
        ...
        R_Tcell = (RC["k_C1_death"] + k_C1_therapy) * V_T_C + RC["k_C_T1"] * V_T_C * V_T_T / (RC["K_T_C"] * V_T_C +
                                                                                              V_T_T + RC["cell"]) * (
            V_T_T / V_T_T + RC["K_T_Treg"] * V_T_Treg + RC["cell"]) * (1 - RC["H_TGFb_Teff"]) * (
            1 - H_PD1_C1)

        antigen_releasing_rate = RC["k_dep"] + (RC["kc_death"] + k_C1_therapy) * V_T_C + RC[
            "k_C_Tcell"] * V_T_C * V_T_T / (
            RC["K_T_C"] * V_T_C + V_T_T + RC["cell"]) * V_T_T / (
            V_T_T + RC["K_T_Treg"] * V_T_Treg + RC["cell"]) * (
        ...
        # 0 -> APC_tumor
        r_APC_death_tumor = RC["k_APC_death"] * \
            (RC["APC0_T"] * V_T - APC_tumor)
        # APC maturation in the tumour
        # APC_tumor ->  mAPC_tumor
        r_APC_maturation_tumor = RC["k_APC_mat"] * \
            c / (c + RC["c50"]) * APC_tumor
        # mAPC death in the tumour
        # mAPC_tumor -> 0
        r_mAPC_death_tumor = RC["k_mAPC_death"] * mAPC_tumor
        # APC recruitment/death in LN
        # 0 -> APC_LN
        r_APC_death_LN = RC["k_APC_death"] * \
            (RC["APC0_LN"] * RC["V_LN"] - APC_LN)
        ...
        if self.atezo != 0:
            ...
            r_PDL1_aPDL1 = 2 * RC["kon_PDL1_aPDL1"] * (PDL1 * aPD1 / RC["gamma_T_aPDL1"]) - RC[
                "koff_PDL1_aPDL1"] * PDL1_aPDL1
        ...
        dV_1_NabPdt = RC["Vmt"] / (V_2_NabP + RC["Kt"]) * V_2_NabP - RC["Vmt"] / (V_1_NabP + RC["Kt"]) * V_1_NabP - RC[
            "Q2"] * V_1_NabP + RC["Q2"] * V_3_NabP - RC["Vmcl"] / (V_1_NabP + RC["Kcl"]) * V_1_NabP
        dV_2_NabPdt = - RC["Vmt"] / (V_2_NabP + RC["Kt"]) * \
            V_2_NabP + RC["Vmt"] / (V_1_NabP + RC["Kt"]) * V_1_NabP
        dV_3_NabPdt = RC["Q2"] * V_1_NabP - RC["Q2"] * V_3_NabP

        daPD1dt = aPD1 - 2 * RC["kon_PDL1_aPDL1"] * \
            (PDL1 * aPD1 / RC["gamma_T_aPDL1"])
        dPDL1_aPDL1dt = r_PDL1_aPDL1 - r_PDL1_PDL1aPDL1
        dPDL1_aPDL1_PDL1dt = r_PDL1_PDL1aPDL1

        self.VT_list.append(V_T)
        self.antigen_releasing_rate_List.append(antigen_releasing_rate)

        res = [dCxdt, dT1_exhdt, dTh_exhdt, dAPC_tumordt, dAPC_LNdt, dmAPC_tumordt, dmAPC_LNdt, dcdt, dV_P_nTregdt,
               dV_LN_nTregdt,
               dV_C_nTregdt, dV_LN_aTregdt, dV_LN_Tregdt, dV_C_Tregdt, dV_P_Tregdt, dV_T_Tregdt, dV_LN_IL2_regdt,
               dV_C_nTdt, dV_P_nTdt, dV_LN_nTdt, dV_LN_aTdt, dV_LN_Tdt, dV_C_Tdt, dV_P_Tdt, dV_T_Tdt,
               dpd1_pdl1dt, dpd2_pdl2dt, dpdl1dt, dV_T_IFNgdt, dV_T_Cdt, dV_1_NabPdt, dV_2_NabPdt, dV_3_NabPdt, daPD1dt,
               dPDL1_aPDL1dt, dPDL1_aPDL1_PDL1dt]
    ...
    def simulate(self, t_start=0, t_end=2):
        ...
        intervals = np.arange(t_end + 1)
        solution = solve_ivp(
            self.immune, [t_start, t_end], self.initials, t_eval=intervals)
        return solution
...
if __name__ == "__main__":
    ...
    No_drug = Immune_systems([1692186.37991838, 121598100.7, 21504017.63, 30088.42153, 1290617.534,
                              ...
                              36.24077612, 2599.365231, 3794.442545, 21367.73076, 765663.9591, 111909867.1, 100511499.8,
                              0.00066611,
                              2038.056155, 101529.8326, 13.19419616, 103609.5265, 1577674.855, 19951265.97, 2915074105,
                              122043960.7, 0.00066611, 21.88935217, 128.7655938, 0.001845759, 1085453188, 100, 0, 0,
                              840, 0, 0], 0, 0)
    Nabp = Immune_systems([1692186.37991838, 121598100.7, 21504017.63, 30088.42153, 1290617.534,
                           ...
                           0.00066611,
                           2038.056155, 101529.8326, 13.19419616, 103609.5265, 1577674.855, 19951265.97, 2915074105,
                           122043960.7, 0.00066611, 21.88935217, 128.7655938, 0.001845759, 1085453188, 100, 0, 0, 840,
                           0, 0], 1, 0)
    ...
    Nabp_Atezo = Immune_systems([1692186.37991838, 121598100.7, 21504017.63, 30088.42153, 1290617.534,
                                 7994.975389, 1119638.044, 1.50E-07, 129486.0973,
                                 36.24077612, 2599.365231, 3794.442545, 21367.73076, 765663.9591, 111909867.1,
                                 100511499.8, 0.00066611,
                                 2038.056155, 101529.8326, 13.19419616, 103609.5265, 1577674.855, 19951265.97,
                                 2915074105, 122043960.7, 0.00066611, 21.88935217, 128.7655938, 0.001845759, 1085453188,
                                 100, 0, 0, 840, 0, 0], 1, 1)
    ...
    x = np.arange(51)
    plt.plot(x, Solution_No_drug.y[24], label="No drug")
    plt.plot(x, Solution_Nabp.y[24], label="Nabp")
    plt.plot(x, Solution_Atezo.y[24], label="Atezo")
    plt.plot(x, Solution_Nabp_Atezo.y[24], label="Combination")
    plt.xlabel("time")
    plt.ylabel("Teff")
    ...
    x = np.arange(51)
    plt.plot(x, Solution_No_drug.y[3], label="No drug")
    plt.plot(x, Solution_Nabp.y[3], label="Nabp")
    plt.plot(x, Solution_Atezo.y[3], label="Atezo")
    plt.plot(x, Solution_Nabp_Atezo.y[3], label="Combination")
    plt.xlabel("time")
    plt.ylabel("APC")
    plt.title('APC Comparison')
    plt.legend(loc="best")
    plt.show()

    T_total = Solution_No_drug.y[15] + Solution_No_drug.y[24] + RC["V_T_Th"]
    V_T_list = ((Solution_No_drug.y[0] + Solution_No_drug.y[29]) * RC["vol_cell"] +
    ...
    V_T_list = ((Solution_Nabp.y[0] + Solution_Nabp.y[29]) * RC["vol_cell"] +
    ...
    V_T_list = ((Solution_Atezo.y[0] + Solution_Atezo.y[29]) * RC["vol_cell"] +
                (Solution_Atezo.y[1] + Solution_Atezo.y[2] + T_total) * RC["vol_Tcell"]) / RC["Ve_T"]
    Atezo_T_VT = V_T_list
    T_total = Solution_Nabp_Atezo.y[15] + \
        Solution_Nabp_Atezo.y[24] + RC["V_T_Th"]
    V_T_list = ((Solution_Nabp_Atezo.y[0] + Solution_Nabp_Atezo.y[29]) * RC["vol_cell"] +
    ...
    if No_drug.nabp != 0:
        k_C1_therapy = RC["k_C_nabp"] * \
            (RC["V_T_NabP"] / (RC["V_T_NabP"] + RC["IC50_nabp"]))
    else:
        k_C1_therapy = 0
    H_PD1_C1 = (Solution_No_drug.y[25] / RC["PD1_50"]) ** RC["n_PD1"] / \
               ((Solution_No_drug.y[25] / RC["PD1_50"]) ** RC["n_PD1"] + 1)
    No_drug_antigen = RC["k_dep"] + (RC["kc_death"] + k_C1_therapy) * Solution_No_drug.y[29] + \
        RC["k_C_Tcell"] * Solution_No_drug.y[29] * Solution_No_drug.y[24] / \
        (RC["K_T_C"] * Solution_No_drug.y[29] + Solution_No_drug.y[24] + RC["cell"]) * \
        Solution_No_drug.y[24] / (
        Solution_No_drug.y[24] + RC["K_T_Treg"] * Solution_No_drug.y[15] + RC["cell"]) * \
        (1 - RC["H_TGFb_Teff"]) * (1 - H_PD1_C1)
    if Nabp.nabp != 0:
        k_C1_therapy = RC["k_C_nabp"] * \
            (RC["V_T_NabP"] / (RC["V_T_NabP"] + RC["IC50_nabp"]))
    ...
    Nabp_antigen = RC["k_dep"] + (RC["kc_death"] + k_C1_therapy) * Solution_Nabp.y[29] + \
        RC["k_C_Tcell"] * Solution_Nabp.y[29] * Solution_Nabp.y[24] / \
        (RC["K_T_C"] * Solution_Nabp.y[29] + Solution_Nabp.y[24] + RC["cell"]) * \
        Solution_Nabp.y[24] / (Solution_Nabp.y[24] + RC["K_T_Treg"] * Solution_Nabp.y[15] + RC["cell"]) * \
        (1 - RC["H_TGFb_Teff"]) * (1 - H_PD1_C1)
    if Atezo.nabp != 0:
        k_C1_therapy = RC["k_C_nabp"] * \
            (RC["V_T_NabP"] / (RC["V_T_NabP"] + RC["IC50_nabp"]))
        ...
        k_C1_therapy = 0
    H_PD1_C1 = (Solution_Atezo.y[25] / RC["PD1_50"]) ** RC["n_PD1"] / \
               ((Solution_Atezo.y[25] / RC["PD1_50"]) ** RC["n_PD1"] + 1)
    Atezo_antigen = RC["k_dep"] + (RC["kc_death"] + k_C1_therapy) * Solution_Atezo.y[29] + \
        RC["k_C_Tcell"] * Solution_Atezo.y[29] * Solution_Atezo.y[24] / \
        (RC["K_T_C"] * Solution_Atezo.y[29] + Solution_Atezo.y[24] + RC["cell"]) * \
        Solution_Atezo.y[24] / (Solution_Atezo.y[24] + RC["K_T_Treg"] * Solution_Atezo.y[15] + RC["cell"]) * \
        (1 - RC["H_TGFb_Teff"]) * (1 - H_PD1_C1)
    if Nabp_Atezo.nabp != 0:
        k_C1_therapy = RC["k_C_nabp"] * \
            (RC["V_T_NabP"] / (RC["V_T_NabP"] + RC["IC50_nabp"]))
    else:
        k_C1_therapy = 0
    H_PD1_C1 = (Solution_Nabp_Atezo.y[25] / RC["PD1_50"]) ** RC["n_PD1"] / \
               ((Solution_Nabp_Atezo.y[25] / RC["PD1_50"]) ** RC["n_PD1"] + 1)
    Nabp_Atezo_antigen = RC["k_dep"] + (RC["kc_death"] + k_C1_therapy) * Solution_Nabp_Atezo.y[29] + \
        RC["k_C_Tcell"] * Solution_Nabp_Atezo.y[29] * Solution_Nabp_Atezo.y[24] / \
        (RC["K_T_C"] * Solution_Nabp_Atezo.y[29] + Solution_Nabp_Atezo.y[24] + RC["cell"]) * \
        Solution_Nabp_Atezo.y[24] / (
        Solution_Nabp_Atezo.y[24] + RC["K_T_Treg"] * Solution_Nabp_Atezo.y[15] + RC[
            "cell"]) * \
        (1 - RC["H_TGFb_Teff"]) * (1 - H_PD1_C1)

    x = np.arange(51)
    plt.plot(x, No_drug_antigen, label="No drug")
    plt.plot(x, Nabp_antigen, label="Nabp")
    plt.plot(x, Atezo_antigen, label="Atezo")
    plt.plot(x, Nabp_Atezo_antigen, label="Combination")
    plt.xlabel("time")
    plt.ylabel("antigen releasing rate")
    plt.title('Antigen Releasing Rate Comparison')
    plt.legend(loc="best")
    plt.show()
