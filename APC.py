...
class Immune_systems():
    def __init__(self, initials, nabp, atezo):
        ...
        self.atezo = atezo
        self.VT_list = []
        self.antigen_relasing_rate_List = []
    ...
    def immune(self, t, Immune):
        ...
        if self.nabp != 0:
            k_C1_therapy = RC["k_C_nabp"]*(RC["V_T_NabP"]/(
                RC["V_T_NabP"]+RC["IC50_nabp"]))*min(V_T_C, RC["Kc_nabp"])/V_T_C
        else:
            k_C1_therapy = 0
        ...
        r_PDL1_trans_in = RC["k_in_PDL1"] * \
            (RC["PDL1_base"] / RC["A_cell"] - RC["PDL1_total"])
        # Secreation of IFNg
        ...
        if self.atezo != 0:
            ...
            r_PD1_aPD1 = 2 * \
                RC["kon_PD1_aPD1"] * (RC["PD1"] * RC["aPD1"]) - \
                RC["koff_PD1_aPD1"] * PDL1_aPDL1
        ...
        return [dCxdt, dT1_exhdt, dTh_exhdt, dAPC_tumordt, dAPC_LNdt, dmAPC_tumordt, dmAPC_LNdt, dcdt,
                dV_e_Pdt, dV_e_pdt, dA_e_Mpdt, dA_s_Mpdt, dA_e_Mdt, dA_s_Mdt, dV_T_Pdt, dV_C_nTregdt, dV_P_nTregdt, dV_LN_nTregdt, dV_LN_aTregdt,
                dV_LN_Tregdt, dV_C_Tregdt, dV_P_Tregdt,  dV_T_Tregdt, dV_LN_IL2_regdt, dV_C_nTdt, dV_P_nTdt, dV_LN_nTdt, dV_LN_aTdt, dV_LN_Tdt, dV_P_Tdt, dV_T_Tdt,
                dV_LN_IL2dt, dV_LN_aThdt, dV_T_Thdt, dV_C_Thdt, dV_P_Thdt, dV_LN_Thdt, dV_T_TGFbdt, dV_T_Cdt, dpd1_pdl1dt, dpd2_pdl2dt, dpdl1_atezodt, dpdl1_atezo_pdl1dt, dpdl1dt, dV_T_IFNgdt, dV_T_Cdt]

    def simulate(self, t_start=0, t_end=50):  
        """
        ...
        intervals = np.linspace(t_start, t_end, 51)
        solution = solve_ivp(
            self.immune, [t_start, t_end], [0]*46, t_eval=intervals)
        return solution
...
"""
    ...
    poly = PolyCollection(verts, facecolors=[color('r'), color('g'), color('b'),
                                             color('y')])
    poly.set_alpha(0.7)
    ax.add_collection3d(poly, Z=Z, zdir='y')
...
def random_walk(num_steps, Teff, antigen, cacner_size):

    start_pos = [Teff[0], antigen[0], cacner_size[0]]
    walk = []
    for i in num_steps:
        walk.appnd[Teff[i-1], antigen[i-1], cacner_size[i-1]]
    return walk


def update_lines(walks, lines):
...
if __name__ == "__main__":
    ...
    No_drug = Immune_systems([1692186.37991838, 121598100.7, 21504017.63, 30088.42153, 1290617.534,
                              ...
                              7780485.275, 1136815835, 232109228.5, 0.121422255, 145.7332167, 21.88935217, 0, 0, 128.7655938, 0.001845759, 1085453188], 0, 0)
    Nabp = Immune_systems([1692186.37991838, 121598100.7, 21504017.63, 30088.42153, 1290617.534,
                           7994.975389, 1119638.044, 1.50E-07, 1.08E-09, 1.03E-10, 0.000360553, 0.054346931, 697.2936537, 23.2432063, 8.99E-10, 129486.0973,
                           36.24077612, 2599.365231, 3794.442545, 21367.73076, 765663.9591, 111909867.1, 100511499.8, 0.00066611,
                           2038.056155, 101529.8326, 13.19419616, 103609.5265, 1577674.855, 19951265.97, 2915074105, 122043960.7, 0.00066611, 107294.4682, 603951.7795,
                           7780485.275, 1136815835, 232109228.5, 0.121422255, 145.7332167, 21.88935217, 0, 0, 128.7655938, 0.001845759, 1085453188], 1, 0)
    Atezo = Immune_systems([1692186.37991838, 121598100.7, 21504017.63, 30088.42153, 1290617.534,
                            7994.975389, 1119638.044, 1.50E-07, 1.08E-09, 1.03E-10, 0.000360553, 0.054346931, 697.2936537, 23.2432063, 8.99E-10, 129486.0973,
                            36.24077612, 2599.365231, 3794.442545, 21367.73076, 765663.9591, 111909867.1, 100511499.8, 0.00066611,
                            2038.056155, 101529.8326, 13.19419616, 103609.5265, 1577674.855, 19951265.97, 2915074105, 122043960.7, 0.00066611, 107294.4682, 603951.7795,
                            7780485.275, 1136815835, 232109228.5, 0.121422255, 145.7332167, 21.88935217, 0, 0, 128.7655938, 0.001845759, 1085453188], 0, 1)
    Nabp_Atezo = Immune_systems([1692186.37991838, 121598100.7, 21504017.63, 30088.42153, 1290617.534,
                                 7994.975389, 1119638.044, 1.50E-07, 1.08E-09, 1.03E-10, 0.000360553, 0.054346931, 697.2936537, 23.2432063, 8.99E-10, 129486.0973,
                                 36.24077612, 2599.365231, 3794.442545, 21367.73076, 765663.9591, 111909867.1, 100511499.8, 0.00066611,
                                 2038.056155, 101529.8326, 13.19419616, 103609.5265, 1577674.855, 19951265.97, 2915074105, 122043960.7, 0.00066611, 107294.4682, 603951.7795,
                                 7780485.275, 1136815835, 232109228.5, 0.121422255, 145.7332167, 21.88935217, 0, 0, 128.7655938, 0.001845759, 1085453188], 1, 1)
...
"""
print("No drug Teff", Solution_No_drug[31])
print("No Drug cancer size", No_drug.VT_list)
print("No Drug antigen releasing rate", No_drug.antigen_relasing_rate_List)


print("Nabp Teff", Solution_Nabp[31])
print("Nabp cancer size", Nabp.VT_list)

print("Atezo antigen releasing rate", Nabp.antigen_relasing_rate_List)


print("Atezo Teff", Solution_Atezo [31])
print("Atezo cancer size", Atezo.VT_list)
#?print("Atezo antigen releasing rate", Atezo.antigen_relasing_rate_List)
#?print("Nabp_Atezo Teff", Solution_Nabp_Atezo.solution[31])
#?print("Nabp_Atezo cancer size", Nabp_Atezo.VT_list)
#?print("Nabp_Atezo antigen releasing rate",
         # Nabp_Atezo.antigen_relasing_rate_List)

"""
   
"""
    Z_Teff = [Solution_No_drug.solution[31], Solution_Nabp.solution[31],
              Solution_Atezo.solution[31], Solution_Nabp_Atezo.solution[31]]
    ...
    Draw_T_effective = figure("number of effective T cells", Z_Teff)
...
"""
    num_steps = 50
    walks = [random_walk(50) for index in range(4)]
    fig = plt.figure()
    ax = fig.add_subplot(project="3d")
    lines = [ax.plot([], [], [])[0] for _ in walks]

    ax.set(xlim3d=(0, 1), xlabel="number of effective T cells")
    ax.set(ylim3d=(0, 1), ylabel="antigen releasing rate")
    ax.set(zlim3d=(0, 1), zlabel="cancer size")
